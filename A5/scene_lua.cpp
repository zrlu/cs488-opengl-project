...
int gr_light_cmd(lua_State* L)
{
  ...
  double col[3];
  get_tuple(L, 1, &l.position[0], 4);
  get_tuple(L, 2, col, 3);
  get_tuple(L, 3, l.falloff, 3);

  if (!lua_isnil(L, 4))
  {
    l.coneAngle = luaL_checknumber(L, 4);
  }

  if (!lua_isnil(L, 5))
  {
    get_tuple(L, 5, &l.coneDirection[0], 3);
  }
...
int gr_material_cmd(lua_State* L)
{
  ...
  if (!lua_isnil(L, 5))
  {
    const char* tex = luaL_checkstring(L, 5);
    data->material->tex = std::string() + tex;
    
    data->material->hasTexture = 1;
  }

  luaL_checktype(L, 6, LUA_TNUMBER);

  double texScaling = luaL_checknumber(L, 6);

  double dparams[4];

  luaL_checktype(L, 7, LUA_TTABLE);
  luaL_argcheck(L, luaL_len(L, 7) == 4, 7, "Four-tuple expected");
  get_tuple(L, 7, dparams, 4);
	...
	for(int i(0); i < 4; ++i) {
      data->material->dparams[i] = dparams[i];
  }

	data->material->shininess = shininess;
  data->material->alpha = alpha;
  data->material->texScaling = texScaling;
...
int gr_node_set_material_cmd(lua_State* L)
{
  ...
  self->material.hasTexture = material->hasTexture;
  self->material.tex = material->tex;
  self->material.dparams = material->dparams;
  self->material.texScaling = material->texScaling;
  return 0;
}

extern "C"
int gr_node_add_light_cmd(lua_State* L)
{
  GRLUA_DEBUG_CALL;
  ...
  gr_light_ud *lightdata = (gr_light_ud *)luaL_checkudata(L, 2, "gr.light");

  luaL_argcheck(L, lightdata != 0, 2, "Light expected");
...
static const luaL_Reg grlib_node_methods[] = {
  ...
  {"add_light", gr_node_add_light_cmd},
